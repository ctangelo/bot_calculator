...
from script import receive_data_and_make_pdf
...
async def start_message(message: types.Message, state=FSMContext):
    ...
    # если вдруг машину состояний запустили, а потом сразу нажали команду старт, то завершаем машину состояний,
    # чтоб не было ошибок
    if current_state:  
        await state.finish()
...
async def import_file(message: types.Message, state: FSMContext):
    ...
    receive_data_and_make_pdf(file, user_id)
    # забираю готовую png из директории
    photo = InputFile(f'/root/doc/{user_id}/Смета.pdf')
    # отправляю pdf
